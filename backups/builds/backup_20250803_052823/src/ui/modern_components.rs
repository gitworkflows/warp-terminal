use iced::widget::{button, container, text, text_input, scrollable, column};
use iced::{theme, Background, Border, Color, Element, Shadow, Vector};
...
impl WarpUI {
    ...
    pub fn section_header<'a, Message>(
        ...
        let mut header = column![
            text(title).size(24).style(Color::from_rgb(0.95, 0.95, 0.95))
        ];
        
        if let Some(sub) = subtitle {
            header = header.push(
                text(sub).size(14).style(Color::from_rgba(0.7, 0.7, 0.7, 0.9))
            );
        }
