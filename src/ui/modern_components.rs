...
impl WarpUI {
    ...
    pub fn scrollable<'a, Message>(content: impl Into<Element<'a, Message>>) -> scrollable::Scrollable<'a, Message> {
        scrollable(content)
    }

    /// Creates a section header for Warp-style themes
    pub fn section_header<'a, Message>(title: &'static str) -> Element<'a, Message> {
        text(title)
            .size(18)
